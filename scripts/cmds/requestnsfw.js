       ...
       const p = getPrefix(event.threadID);
		...
		if (fs.existsSync(pendingIDsPath)) {
			...
			if (pendingIDs.includes(threadID)) {
				const pendingMsg = "Your request is already in pending.... \nPlease contact Walex󱢏 for fast approval.\n\n FB: https://www.facebook.com/P.walex.I\n\n or join the Marin support box for help\nType :${p}supportgc or walegc or walegc2 \nto join";
...
for (const adminID of config.adminBot) {
  api.sendMessage(msg, adminID);
		...
		const notifyMsg = `✅Your approval request has been sent to admin 𝒘𝒂𝒍𝒆𝒙.󱢏 with this\nmessage: ${userMessage}.\n\n I will notify you if your thread is approved. Please wait until then.\n\n join the Marin supportgc box for fast approve \ntype :{p}supportgc or walegc or walgegc2 \nto join`;
